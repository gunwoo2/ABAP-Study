...
*&
*&---------------------------------------------------------------------*
*& Exercise 35 Open SQL - Group by
*&
*&---------------------------------------------------------------------*

REPORT zbc402_e03_aggregate.
...
CLASS lcl_customer DEFINITION.
  ...
  PRIVATE SECTION.
    ...
    TYPES: BEGIN OF ty_s_sums,
             forcuram  TYPE sbook-forcuram, "금액
             forcurkey TYPE sbook-forcurkey, "통화 코드(나라 2자리 + 그 나라에서 사용하는 영어 한자리)
           END OF ty_s_sums.
...
* calculate discount
  ...
  METHOD set_sums.
    ...
    " 인스턴스 어트리뷰트인 MT_SUMS를
    " 통화별 예약 합계 금액으로 채우고자 한다.

    " 데이터는 Database Table인 SBOOK에서 가져온다.
    " 조건은 고객번호가 인스턴스 어트리뷰트인 MV_CUSTOMID와 같은
    " 데이터만 검색하도록 한다.

    " 데이터를 가져올때는 통화별로 합산하여 MT_SUMS에 채운다.

     SELECT SUM( FORCURAM ) "숫자(금액)
                FORCURKEY   "문자(통화키)
       FROM SBOOK
       INTO TABLE MT_SUMS
       WHERE CUSTOMID EQ MV_CUSTOMID
            AND CANCELLED EQ SPACE " 상수 SPACE는 공백을 의미함.
            AND AGENCYNUM NE SPACE " 여행사 정보가 있는 데이터만 검색
       GROUP BY FORCURKEY.
