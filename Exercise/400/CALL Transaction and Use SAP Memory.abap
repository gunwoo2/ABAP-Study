...
*&
*&---------------------------------------------------------------------*
*& Exercise 33 CALL Transaction and Use SAP Memory
*&
*&---------------------------------------------------------------------*

REPORT  zbc400_e03_conn_list_MEM
...
*        CALL FUNCTION '함수명'.
*        CALL TRANSACTION '티코드'.
*        CALL METHOD

        CALL TRANSACTION 'BC402MCAR' "T-code BC402MCAR 호출
             AND SKIP FIRST SCREEN. "첫 화면은 SKIP

       WHEN OTHERS.

        " 더블클릭한 라인의 항공사의 특정 항공편을 자세하게
      " 출력하기 위한 프로그램 호출
        SUBMIT BC402_INS_FLIGHT_LIST_OSQL
                WITH PA_CAR = LS_CONN-CARRID
                WITH CO_CAR = LS_CONN-CONNID
                AND RETURN.
